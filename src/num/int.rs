//! Nubmer parsing helper libraries
    ...
    ($name:ident, $name_int:ident, $ty:ty) => {
        pub fn $name(s: &str) -> crate::Result<$ty> {
            ...
            match components.as_slice() {
                [Expr::Val(ret)] => Ok(*ret),
                _ => Err(crate::Error::Num(Error::InvalidExpression)),
            }
        }
    }
}
...
macro_rules! int_parse {
    ($name:ident, $name_int:ident, $ty:ty) => {
        ...
        pub fn $name_int(mut s: &str) -> crate::Result<$ty> {
            ...
            // Parse the integer
            <$ty>::from_str_radix(s, base)
                .map_err(crate::num::Error::ParseSigned)
                .map_err(crate::Error::Num)
...
macro_rules! uint_parse {
    ($name:ident, $name_int:ident, $ty:ty) => {
        ...
        pub fn $name_int(mut s: &str) -> crate::Result<$ty> {
            ...
            // Parse the integer
            <$ty>::from_str_radix(s, base)
                .map_err(crate::num::Error::ParseUnsigned)
                .map_err(crate::Error::Num)
        }
